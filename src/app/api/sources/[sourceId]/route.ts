...
function normalize(payload: SourceRequest) {
  ...
  const mapQueue = (queue: "upcoming" | "in_transit") =>
    Object.fromEntries(
      Object.entries(payload.mapping?.[queue] ?? {})
  ...
  return {
    ...
    upcomingTab,
    inTransitTab,
    mapping: { upcoming: mapQueue("upcoming"), in_transit: mapQueue("in_transit") },
...
export async function PUT(request: Request, { params }: { params: Promise<{ sourceId: string }> }) {
  try {
    const source = await updateSource((await params).sourceId, normalize(await request.json()));
    return source ? NextResponse.json({ source }) : NextResponse.json({ error: "Source not found." }, { status: 404 });
  } catch (error) {
...
export async function DELETE(_: Request, { params }: { params: Promise<{ sourceId: string }> }) {
  try {
    const ok = await deleteSource((await params).sourceId);
    return ok
